...
def send_read(client_socket, fileserverIP_DS, fileserverPORT_DS, filename , RW, file_version_map, msg, filename_DS, client_id):
    ...
    if filename not in file_version_map:
        ...
        send_msg = filename + "|" + RW + "|" + msg
        print(str(fileserverIP_DS) + " " + str(fileserverPORT_DS))
        client_socket.connect((fileserverIP_DS,fileserverPORT_DS))
        client_socket.send(send_msg.encode())
    ...
    if version_FS != str(file_version_map[filename]):
        ...
        print("File server IP: "+fileserverIP_DS)
        ...
        print("File server IP: "+fileserverIP_DS)
        text = cache(filename_DS, "READ", "r", client_id)

    return True,text     # went to cache
...
def handle_write(filename, client_id, file_version_map, text):
    # ------ INFO FROM DS ------
    ...
    if reply_DS == "FILE_DOES_NOT_EXIST":
        ...
        # ------ WRITING TO FS ------
        client_socket = create_socket()
        send_write(client_socket, fileserverIP_DS, int(fileserverPORT_DS), filename_DS, "w", file_version_map, write_client_input) # send text and filename to the fileserver
        ...
        print(reply_FS)    # split version num from success message and print message
        ...
        # ------ CACHING ------
        cache(filename_DS, write_client_input, "w", client_id)
...
def cache(filename_DS, write_client_input, RW, client_id):
    ...
    cache_file = os.path.join(curr_path, "client_cache", client_id)
    ...
    distutils.dir_util.mkpath(cache_file)


    if RW == "w" or RW == "w":
        with open(os.path.join(curr_path, "client_cache", client_id,filename_DS), RW) as f:        # write to the cached file
            f.write(write_client_input)
    else:
        with open(os.path.join(curr_path, "client_cache", client_id,filename_DS), "r") as f:        # read from the cached file
            print_breaker()
            return f.read()
            print_breaker()
...
def handle_read(filename, file_version_map, client_id):
    ...
    if reply_DS == "FILE_DOES_NOT_EXIST":
        ...
        read_cache,text = send_read(client_socket, fileserverIP_DS, int(fileserverPORT_DS), filename_DS, "r", file_version_map, "READ", filename_DS, client_id) # send filepath and read to file server
